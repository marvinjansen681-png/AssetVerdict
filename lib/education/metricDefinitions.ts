...
export const METRIC_DEFINITIONS: Record<string, MetricDefinition> = {
  ...
  capRatePP: def({
    ...
    category: "valuation",
    perspective: "property",
    simpleExplanation: "The property's operating income as a percentage of what you're paying for it.",
  ...
  capRateMV: def({
    ...
    category: "valuation",
    perspective: "property",
    simpleExplanation: "The property's operating income as a percentage of its current market value, rather than what you're paying.",
    whyItMatters: "Comparing this to Cap Rate on Purchase Price shows whether you're buying below, at, or above market value.",
  ...
  capRateSpread: def({
    ...
    category: "valuation",
    perspective: "property",
    simpleExplanation: "How much better (or worse) your deal's cap rate is compared to the going market rate for similar property.",
    whyItMatters: "A positive spread is a signal you're buying below what the market normally pays for this kind of income stream.",
    formulaLabel: "Cap Rate (MV) − Market Cap Rate",
    formulaExplanation: "AssetVerdict subtracts the market cap rate you've entered from this deal's Cap Rate on Market Value.",
    preferredDirection: "higher",
  ...
  irr: def({
    ...
    category: "returns",
    perspective: "investor",
    simpleExplanation: "The single annualised return this deal is projected to deliver on YOUR OWN CASH over 20 years, after financing and tax, once you include the eventual sale.",
    ...
    formulaExplanation:
      "AssetVerdict builds one cash-flow stream: -Initial Equity Investment at year 0, then 20 years of after-debt-service, after-tax cashflow (with rent, cost, and capital growth escalations), with the after-tax, after-debt-payoff sale proceeds added to year 20. It solves for the discount rate that makes that stream's present value zero. Financing changes this number twice over — once through the smaller initial outlay (equity, not the full purchase price), and once through debt service reducing every year's cashflow — so leverage can push Equity IRR up OR down depending on whether the property's own return beats the cost of the debt.",
    preferredDirection: "higher",
    ...
    commonMistake: "Assuming more leverage always means a higher IRR — it amplifies whatever the underlying spread is between the property's return and the cost of debt, in either direction. Also: IRR assumes you hold for 20 years and reinvest at the same rate, so real outcomes depend heavily on the growth assumptions you fed in. Not applicable with no (or negative) equity invested.",
  ...
  grossProfit: def({
    ...
    category: "flip",
    perspective: "flip",
    simpleExplanation: "What you'd walk away with if there were no capital gains tax.",
    whyItMatters: "It's the headline profit figure before the tax authority takes its share.",
  ...
  netProfit: def({
    ...
    category: "flip",
    perspective: "flip",
    simpleExplanation: "What you actually walk away with, after capital gains tax.",
    whyItMatters: "It's the real, bottom-line answer to 'was this flip worth it?'",
    formulaLabel: "Gross Profit − Capital Gains Tax",
    formulaExplanation: "AssetVerdict applies your capital gains tax rate to gross profit (floored at zero — no tax benefit is modelled for a loss-making flip) and subtracts it.",
    preferredDirection: "higher",
    affectedBy: ["grossProfit"],
    affects: ["roi", "annualisedROI", "profitMargin"],
    relatedMetrics: ["grossProfit", "roi"],
    investorQuestion: "After tax, what does this flip actually put in my pocket?",
  ...
  roi: def({
    ...
    category: "flip",
    perspective: "flip",
    simpleExplanation: "Your net profit as a percentage of everything you spent to make it.",
    whyItMatters: "It lets you compare this flip's return against other flips or investments, independent of deal size.",
    formulaLabel: "Net Profit ÷ Total Cost",
    formulaExplanation: "AssetVerdict divides net profit by total cost.",
    preferredDirection: "higher",
    affectedBy: ["netProfit", "totalCost"],
    affects: ["annualisedROI"],
    relatedMetrics: ["annualisedROI", "profitMargin"],
    commonMistake: "Comparing ROI across flips with very different holding periods without also checking Annualised ROI.",
    investorQuestion: "What percentage return did my capital earn on this flip?",
    strategies: ["Increase net profit", "Reduce total cost"],
  }),
  ...
  annualisedROI: def({
    ...
    category: "flip",
    perspective: "flip",
    simpleExplanation: "ROI adjusted for how long the flip took, so deals of different lengths can be compared fairly.",
    whyItMatters: "A 20% ROI over 3 months is a much better use of capital than the same 20% over 18 months — this metric makes that visible.",
    formulaLabel: "ROI ÷ Holding Period (years)",
    formulaExplanation: "AssetVerdict divides ROI by the holding period expressed in years.",
    preferredDirection: "higher",
    affectedBy: ["roi", "holdingCosts"],
    affects: [],
    relatedMetrics: ["roi", "holdingCosts"],
    investorQuestion: "On a like-for-like annual basis, how good is this flip's return?",
    strategies: ["Increase ROI", "Shorten the holding period"],
  }),
  ...
  profitMargin: def({
    ...
    category: "flip",
    perspective: "flip",
    simpleExplanation: "Your net profit as a percentage of the sale price, not the cost.",
    whyItMatters: "It shows how much cushion you have if the sale price comes in lower than expected.",
    formulaLabel: "Net Profit ÷ Expected Sale Price",
    formulaExplanation: "AssetVerdict divides net profit by the expected sale price.",
    preferredDirection: "higher",
    ...
    strategies: ["Increase net profit", "Avoid over-improving relative to the achievable sale price"],
  }),
};
