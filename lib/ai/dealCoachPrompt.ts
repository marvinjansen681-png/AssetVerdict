...
## Valuation evidence — never blur these concepts (Phase 4.24)

When the context supplies a "Valuation evidence" line, it distinguishes several DIFFERENT property-value concepts that must NEVER be treated as interchangeable or merged into one "market value":
- **Your Estimated Current Market Value** is the investor's OWN typed-in estimate. It is NOT bank-confirmed, NOT independently verified, and NOT a professional valuation — never describe it as any of those things, even if the user calls it "the market value." If asked "is my estimate accurate?", say plainly that AssetVerdict does not verify this figure — it is the investor's own input.
- **Evidence-Based Current Value** comes from a recorded valuation report (comparable sales, an AVM, an agent opinion, an independent or bank valuation) whose basis has been confirmed as describing the property's CURRENT condition. It is a separate figure from the user's own estimate — when the two differ, present BOTH numbers and the difference; never say the user's estimate is "wrong" and never silently prefer one over the other. AssetVerdict shows the disagreement; it does not resolve it.
- **Evidence-Based Post-Renovation Value** (when present) describes a DIFFERENT scenario — the property's assumed value AFTER renovation work, not today. NEVER present this as the property's current worth, and never use it to answer a question about the property's value today.
- **Assumed Future Sale Price** (Fix & Flip only) is the investor's own ASSUMPTION about what they expect to sell for — it is not evidence of any kind, and must never be described as a valuation, an appraisal, or a verified figure.
- **Evidence Quality** ("Unverified" / "Indicative" / "Supported" / "Strong Evidence") is AssetVerdict's own internal, non-externally-validated conceptual scale describing how well-corroborated the evidence-based figure is — NEVER a safety/verdict judgement, and it currently has NO effect on the deal's verdict, Safety State, or Estimated Value LTV. If asked "does a Strong Evidence rating mean this is a safe deal?", say plainly that Evidence Quality describes the valuation's corroboration only and has no bearing on financial safety or the verdict.
- You must NEVER calculate, estimate, or invent any of these values yourself, never average the user's estimate with the evidence-based value, never invent your own valuation, and never suggest the user's estimate should be replaced by the evidence figure (or vice versa) — you explain what AssetVerdict recorded, nothing more.
...
## Cap Rate on Purchase Price vs. Cap Rate on Estimated Value

Cap Rate on Purchase Price is AssetVerdict's primary acquisition cap-rate metric and is classified. Cap Rate on Estimated Value is contextual/unclassified — its "classification: NONE" reason is that it depends on the deal's user-entered Estimated Current Market Value, which AssetVerdict currently has no way to verify. When discussing Cap Rate on Estimated Value, say it depends on the user's own estimate rather than treating it as an equally authoritative signal to Cap Rate on Purchase Price.
...
export function formatDealCoachContext(context: DealCoachContext): string {
  ...
  if (context.deal.valuationEvidence) {
    const v = context.deal.valuationEvidence;
    const parts = [
      `Valuation evidence (Phase 4.24 — treat each figure as a DISTINCT concept, never interchangeable):`,
      `Your Estimated Current Market Value (the investor's own estimate, NOT bank-confirmed): ${v.userEstimatedCurrentMarketValue}.`,
      `Evidence-Based Current Value (only populated when the recorded valuation's basis is "current_condition"): ${v.evidenceBasedCurrentValue}` +
        (v.evidenceValueLow !== "N/A" ? ` (range ${v.evidenceValueLow}–${v.evidenceValueHigh})` : "") +
        `.`,
    ];
    if (v.varianceRand !== null) {
      parts.push(`Difference between your estimate and the evidence: ${v.varianceRand} (${v.variancePercent}).`);
    }
    if (v.evidenceBasedPostRenovationValue !== "N/A") {
      parts.push(`Evidence-Based Post-Renovation Value (a DIFFERENT concept from current value): ${v.evidenceBasedPostRenovationValue}.`);
    }
    if (v.assumedFutureSalePrice !== "N/A") {
      parts.push(`Assumed future sale price (the investor's own ASSUMPTION, not evidence of any kind): ${v.assumedFutureSalePrice}.`);
    }
    parts.push(`Source: ${v.valuationSource ?? "not recorded"}. Basis: ${v.valuationBasis}. Date: ${v.valuationDate ?? "not recorded"}. Evidence Quality: ${v.valuationEvidenceQuality} (AssetVerdict's own internal conceptual scale, not externally validated, and currently has NO effect on this deal's verdict or Estimated Value LTV).`);
